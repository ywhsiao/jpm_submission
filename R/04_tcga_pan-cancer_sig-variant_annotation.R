...
library(dbplyr)
library(DBI)
revel <- dbConnect(RSQLite::SQLite(), dbname= 'revel_hg38.db')
dbListTables(revel)
refdb <- tbl(revel, "revel_hg38")
...
ggp_revel <- ggplot(revel, aes(x = REVEL)) +
  ...
  theme_bw()
ggsave('revelscore_byrace.png', width = 7, height = 7, dpi = 700)
...
ggsave('clinvar_byrace.png', width = 14, height = 20, dpi = 700)
...
ggsave('tcga_pan-cancer_validation_db_figureS7.png',width = 35, height =20, units = 'cm', dpi = 700)
