...
figure <- ggarrange(p, g,
                    ...
                    heights = c(1, 0.5),
                    ncol = 1, nrow = 2)
ggsave('tcga_pan-cancer_hrd_vs_tmb_figureS1.png',width = 35, height = 30, units = 'cm')
...
my_comparisons <- list(c('White','Asian'), c('Asian', 'African American/Black'),
                       c('White', 'African American/Black'))
ggp_hrd <- ggboxplot( df, x ="race", y = "HRD_score", 
  ...
  stat_compare_means(label.y = y+interval*4-3) +rremove("x.text")+rremove("x.ticks")+theme(legend.position="bottom")
ggp_hrd
ggsave(paste0("tcga_pan-cancer_HRDscores_racial_comparison.png"),width = 40, height =30, units = 'cm')
...
my_comparisons <- list(c('White','Asian'), c('Asian', 'African American/Black'),
                       c('White', 'African American/Black'))
ggp_tmb <- ggboxplot( df, x ="race", y = "TMB_value", 
                  facet.by = "acronym", ylim = c(0, y+interval*4),
                  color = "race", palette = "jco", xlab = FALSE,legend = "right") +
  stat_pvalue_manual(final_data, label = "p.adj") +
  stat_compare_means(label.y = y+interval*4-3)+rremove("x.text")+rremove("x.ticks")+theme(legend.position="bottom")
ggsave(paste0("tcga_pan-cancer_TMBvalue_racial_comparison.png"),width = 40, height =30, units = 'cm')

# figure S2
figure <- ggarrange(ggp_hrd, ggp_tmb,
                    labels = c("A", "B"),
                    font.label = list(size = 20),
                    common.legend = T,
                    legend = "bottom",
                    ncol = 1, nrow = 2)
ggsave('tcga_pan-cancer_hrd_vs_tmb_figureS2.png',width = 35, height = 60, units = 'cm')
