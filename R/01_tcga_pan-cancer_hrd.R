...
for (i in 1:5){
  if(i ==2) next
  geneset_id = set_ID[i]
  clinic_hrd_geneset <- read.csv(paste0('public_data_2/cnv-based/',geneset_id,'_clinic_plus_hrd.csv'), header = T, stringsAsFactors = F)
  to_stacked_bar_plot <- clinic_hrd_geneset[,c(1,2,12:14)]
  to_stacked_bar_plot$acronym <- as.factor(to_stacked_bar_plot$acronym)
  to_stacked_bar_plot <- aggregate(to_stacked_bar_plot[,3:5], by=list(to_stacked_bar_plot$acronym), FUN=mean)
  names(to_stacked_bar_plot)[1] <- 'acronym'
...
b <- base_plot +
  ...
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))
b
ggsave('public_data_2/cnv-based/tcga_pan-cancer_hrd-score_stacked-bar-plot.png',width = 35, height =10, units = 'cm')
...
drawPairwiseBoxPlot <- function(geneset_id, y, interval){
  ...
  ggsave(paste0("public_data_2/cnv-based/",geneset_id,"_HRDscores_racial_comparison.png"),width = 30, height =30, units = 'cm')
}
...
for (i in 1:5){
  ...
  write.csv(coef_df, paste0('public_data_2/cnv-based/',geneset_id,'_multi-cox_coef.csv'))
  pvalue_df <- Reduce(rbind, pvalue_list)
  write.csv(pvalue,paste0('public_data_2/cnv-based/',geneset_id,'_multi-cox_pvalue.csv'))
}
getwd()
# all cancer
for (i in 1:5){
  ...
  pvalue <- as.data.frame(rbind(x$logtest, x$sctest, x$waldtest))
  row.names(pvalue) <- c('logtest', 'sctest', 'waldtest')
  write.csv(coefs, paste0('public_data_2/cnv-based/',geneset_id,'_pan-cancer_multi-cox_coef.csv'))
  write.csv(pvalue,paste0('public_data_2/cnv-based/',geneset_id,'_pan-cancer_multi-cox_pvalue.csv'))
}

# plot forest plots
setwd('public_data_2/cnv-based/')
library(data.table)
...
ggsave('tcga_pan-cancer_hrd-score_figure1.png',width = 35, height =20, units = 'cm', dpi = 700)
...
surv_df_final0 <- rbind(surv_df, surv_df_pan)
surv_df_final0 <- surv_df_final0[surv_df_final0$acronym !='SKCM',]
toexport <- surv_df_final0 %>%
...
hrdFocus <- c('OV','Pan-cancer')
surv_df_final <- surv_df_final0[surv_df_final0$acronym %in% hrdFocus,]
...
df$geneset <- factor(df$geneset, levels=c('genomewide', 'DDR_276', 'HR_PARP_132', 'DDRD_assay_42'))
df$acronym <- factor(df$acronym, levels=c('OV','Pan-cancer'))
fit1 <-  survfit(Surv(overall.survival, vital_status) ~HRD_score, data = df)
names(surv_df)
ggsurv <- ggsurvplot(fit1, df, facet.by = c("acronym","geneset"),
                     palette = "jco", pval = TRUE, pval.size = 20)
p1 <- ggsurv+ geom_text(
  ...
  vjust   = -18,
  size = 4)
ggsave("cnv-based/tcga_pan-cancer_hrd-score_survival_curve_ov_pan.png",width = 35, height = 15, units = 'cm', dpi = 700)

# select cancer cohorts to present
hrdFocus <- c('BRCA', 'PAAD' ,'PRAD')
#hrdFocus <- c('OV','Pan-cancer')
surv_df_final <- surv_df_final0[surv_df_final0$acronym %in% hrdFocus,]
surv_df_final$overall.survival <- as.numeric(surv_df_final$overall.survival)
surv_df_final$HRD_score <- as.factor(surv_df_final$HRD_score)
df <- as_tibble(surv_df_final)
df$geneset <- factor(df$geneset, levels=c('genomewide', 'DDR_276', 'HR_PARP_132', 'DDRD_assay_42'))
df$acronym <- factor(df$acronym, levels=c('BRCA', 'PAAD' ,'PRAD'))
fit1 <-  survfit(Surv(overall.survival, vital_status) ~HRD_score, data = df)
names(surv_df)
ggsurv <- ggsurvplot(fit1, df, facet.by = c("acronym","geneset"),
                     palette = "jco", pval = TRUE, pval.size = 20)
p2 <- ggsurv+ geom_text(
  ...
  vjust   = -20,
  size = 4)
ggsave("cnv-based/tcga_pan-cancer_hrd-score_survival_curve_rest_three.png",width = 35, height = 23, units = 'cm', dpi = 700)
...
ggarrange(p1, p2, 
          ...
          heights = c(1.2,1.8),
          font.label = list(size = 18, color = "black", face = "bold", family = NULL))
ggsave("cnv-based/tcga_pan-cancer_hrd-score_survival_curve_merged.png",width = 35, height = 40, units = 'cm', dpi = 700)
...
ggsave("cnv-based/tcga_pan-cancer_hrd-score_cox_forestpolt.png",width = 50, height = 20, units = 'cm', dpi = 700)
